#!/bin/bash
# AgenticCodebase — one-liner install script
...
download_binary() {
    ...
    local asset_name="agentic-codebase-${version_num}-${platform}.tar.gz"
...
install_from_source() {
    ...
    if [ "$DRY_RUN" = true ]; then
        echo "  [dry-run] Would run: cargo install --git ${git_url} ${source_ref_text}--locked --force agentic-codebase"
        echo "  [dry-run] Would copy from ${cargo_bin}/(acb,${BINARY_NAME}) to ${INSTALL_DIR}/"
        return
    fi
    ...
    chmod +x "${INSTALL_DIR}/acb" "${INSTALL_DIR}/${BINARY_NAME}"
    echo "  Installed from source to ${INSTALL_DIR}/acb and ${INSTALL_DIR}/${BINARY_NAME}"
}
...
configure_codex() {
    ...
    local codex_cmd_args=("${MCP_ENTRYPOINT}")
...
print_post_install_next_steps() {
    ...
    if [ "$PROFILE" = "server" ]; then
        ...
        echo "  5. Optional feedback: open https://github.com/agentralabs/codebase/issues" >&3
    else
        echo "  2. Restart your MCP client/system so it reloads MCP config." >&3
        echo "  3. ${SERVER_KEY} now auto-detects local .acb files at startup when available." >&3
...
main() {
    ...
    echo "AgenticCodebase Installer"
    echo "========================="
    echo ""
    ...
    finish_progress
    echo "Install complete: AgenticCodebase (${PROFILE})" >&3
    echo "" >&3
    if [ "$PROFILE" = "desktop" ]; then
        echo "Done! Restart any configured MCP client to use AgenticCodebase." >&3
    else
        echo "Done! AgenticCodebase install completed." >&3
    fi
    print_post_install_next_steps
